...
for np_name in glob.glob('../data_acrobot_softmax/acrobot_softmax_NG - 1*.npy'):
    pg.append(np.load(np_name))

for np_name in glob.glob('acrobot_born_NG_ - 1*.npy'):
...
plt.plot(smoothed_pg,label="QNPG softmax",color="darkblue",alpha=0.7)
